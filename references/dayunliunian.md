...
When `bash_tool` is available, cross-check 起运年龄 and the 大运 sequence against `lunar-python`'s `EightChar.getYun(gender)` (see `paipan.md`) rather than relying on manual day-counting alone — the library computes 起运 down to the day, which is more precise than the "3天=1岁" approximation in §1. 排流年 (§2) doesn't need cross-validation — it's a simple 60-cycle lookup with no precision issues. The interpretive layer (§3–4: 天克地冲/冲提纲/岁运并临/宫位映射) has no library equivalent and remains this file's own analytical framework.