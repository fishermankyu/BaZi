...
13. **If asked about a specific 神煞 (桃花/驿马/华盖/etc.), 空亡, or 纳音** — see `references/shenshakongwang.md`. Look up the relevant marker(s) using the fixed lookup rules there, and read 神煞/空亡 results relative to whether they land on a 喜用神 or 忌神 per step 11's conclusion. 纳音 (Na Yin) is decorative/folk-context information (mainly used for 合婚) — mention only when directly asked, don't fold it into the structural reading. All opt-in — supplementary color, not part of the core structural read, and never overrides steps 5–11's conclusions.
...
- `references/shenshakongwang.md` — 神煞、空亡与纳音 (Special Stars, Void & Na Yin) rules: fixed lookup tables for 9 common 神煞 (天乙贵人/文昌/桃花/驿马/华盖/将星/魁罡/羊刃/天德月德), the 六旬空亡 lookup and its modifiers (冲/合/填实), the full 60-甲子纳音 table (verified against lunar-python), and how these read relative to 喜用神/忌神. Opt-in, supplementary color only.
...
| Library cross-validation methods (`sxtwl`/`lunar-python`/`cnlunar` usage patterns) | `paipan.md` | `stems_branches.md`, `hidden_stems.md`, `ten_gods.md`, `shenshakongwang.md` (空亡), `dayunliunian.md` (起运/大运) — each has a "Cross-validation" section pointing back here; if the recommended library/API changes, check all five |
